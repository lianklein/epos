...
public:
      ...
      // Heap regular
      Segment * segment = new (&Application::_preheap[0]) Segment(Traits<Application>::HEAP_SIZE);
      CPU::Log_Addr addr = as.attach(*segment);
      Application::_heap = new (&Application::_preheap[0]) Heap(addr, segment->size());

      //Heap uncached
      MMU::Flags flags = MMU::Flags::CD | MMU::Flags::APP; // Desativar caching
      Segment * uncached_segment = new (&Application::_uncached_preheap[0]) Segment(Traits<Application>::HEAP_SIZE, flags);
      CPU::Log_Addr addr_uncached = as.attach(*uncached_segment);
      Application::_uncached_heap = new (&Application::_uncached_preheap[0]) Heap(addr_uncached, uncached_segment->size());
